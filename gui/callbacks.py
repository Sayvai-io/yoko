...
class GUIState:
    ...
    def __init__(self) -> None:
        ...
        self.canvas_aspect_ratio = 1500.0 / 900  # Millimiter paper
        self.w_rel_body_size = 0.5  # Body size as fraction of horisontal canvas axis
        self.h_rel_body_size = 0.95
        self.background_body_scale = (
            1 / 171.99
        )  # Inverse of the mean_all body height from GGG
        self.background_body_canvas_center = (
            0.273  # Fraction of the canvas (millimiter paper)
        )
        self.w_canvas_pad, self.h_canvas_pad = 0.011, 0.04
        self.body_outline_classes = (
            ''  # Application of pattern&body scaling when it overflows
        )
    ...
    def layout(self):
        ...
        with ui.row(wrap=False).classes(
            f'w-full h-[{self.h_params_content}dvh] p-0 m-0 '
        ):
            # Tabs
        ...
        with ui.header(elevated=True, fixed=False).classes(
            f'h-[{self.h_header}vh] items-center justify-end py-0 px-4 m-0'
        ):
            ui.label('GarmentCode design configurator').classes('mr-auto').style(
                'font-size: 150%; font-weight: 400'
            )
            ui.button(
                'About the project',
                on_click=lambda: ui.navigate.to(
                    'https://igl.ethz.ch/projects/garmentcode/', new_tab=True
                ),
            ...
            ui.button(
                'Dataset',
                on_click=lambda: ui.navigate.to(
                    'https://igl.ethz.ch/projects/GarmentCodeData/', new_tab=True
                ),
            ).props('flat color=white')
            with ui.link(
        ...
        with ui.footer(fixed=False, elevated=True).classes(
            ...
            ui.link(
                '© 2024 Interactive Geometry Lab', 'https://igl.ethz.ch/', new_tab=True
            ).classes('text-white')

    def view_tabs_layout(self):
        '''2D/3D view tabs'''
        with ui.column(wrap=False).classes(
            f'h-[{self.h_params_content}vh] w-full items-center'
        ):
            ...
            with ui.tab_panels(tabs, value=self.ui_2d_tab, animated=True).classes(
                ...
                with ui.tab_panel(self.ui_2d_tab).classes(
                    'w-full h-full items-center justify-center p-0 m-0'
                ):
                    self.def_pattern_display()
                with ui.tab_panel(self.ui_3d_tab).classes(
                    'w-full h-full items-center p-0 m-0'
                ):
                    self.def_3d_scene()
    ...
    def def_param_tabs_layout(self):
        ...
        with ui.column(wrap=False).classes(f'h-[{self.h_params_content}vh]'):
            ...
            with ui.tab_panels(tabs, value=self.ui_parse_tab, animated=True).classes(
                'w-full h-full items-center'
            ):  # Changed default value to parse tab
                with ui.tab_panel(self.ui_parse_tab).classes(
                    'w-full h-full items-center p-0 m-0'
                ):
                    self.def_parse_tab()
                with ui.tab_panel(self.ui_design_tab).classes(
                    'w-full h-full items-center p-0 m-0'
                ):
                    self.def_design_tab()
                with ui.tab_panel(self.ui_body_tab).classes(
                    'w-full h-full items-center p-0 m-0'
                ):
                    self.def_body_tab()
    ...
    def def_body_tab(self):
        ...
        ):  # NOTE: p-0 m-0 gap-0 dont' seem to have effect
            ...
            if 'v' not in design_params[param]:
                    ...
                    with ui.expansion().classes('w-full p-0 m-0') as expansion:
                        with expansion.add_slot('header'):
                            ui.label(f'{param_name}').classes(
                                'text-base self-center w-full h-full p-0 m-0'
                            )
                        with ui.row().classes(
                            'w-full h-full p-0 m-0'
                        ):  # Ensures correct application of style classes for children
                            self.def_flat_design_subtab(
                                ui_elems[param], design_params[param]
                            )
                else:
                ...
                if 'select' in p_type:
                    ...
                    ui.label(param_name).classes(
                        'p-0 m-0 mt-2 text-stone-500 text-[0.85rem]'
                    )
                    ui_elems[param] = ui.select(
                        values,
                        value=val,
                        on_change=lambda e, dic=design_params, param=param: self.update_pattern_ui_state(
                            dic, param, e.value
                        ),
                    ).classes('w-full')
                elif p_type == 'bool':
                    ...
                    ui_elems[param] = (
                        ...
                        .on(
                            'update:model-value',
                            lambda e, dic=design_params, param=param: self.update_pattern_ui_state(
                                dic, param, e.args
                            ),
                            throttle=0.5,
                    ...
                    print(
                        f'GUI::NotImplementedERROR::{param}::'
    ...
    def def_design_tab(self):
        ...
        if self.pattern_state.is_design_sectioned():
            ...
            with ui.splitter(value=self.w_splitter_design).classes(
                'w-full h-full p-0 m-0'
            ) as splitter:
                ...
                with splitter.after:
                    with ui.tab_panels(
                        tabs, value=self.ui_design_subtabs['meta']
                    ).props('vertical').classes('w-full h-full p-0 m-0'):
                        for param, tab_elem in self.ui_design_subtabs.items():
                            with ui.tab_panel(tab_elem).classes(
                                'w-full h-full p-0 m-0'
                            ).style('gap: 0px'):
                                with ui.scroll_area().classes(
                                    'w-full h-full p-0 m-0'
    ...
    def def_pattern_display(self):
        ...
        with ui.column().classes('h-full p-0 m-0'):
            with ui.row().classes('w-full p-0 m-0 justify-between'):
                ...
                self.ui_self_intersect = (
                    ui.label('WARNING: Garment panels are self-intersecting!')
                    .classes(
                        'font-semibold text-purple-600 border-purple-600 border py-0 px-1.5 rounded-md'
                    )
            ...
            with ui.image(
                ...
                f'aspect-[{self.canvas_aspect_ratio}] h-[95%] p-0 m-0'
            ) as self.ui_pattern_bg:
                # NOTE: Positioning: https://github.com/zauberzeug/nicegui/discussions/957
                with ui.row().classes(
                    'w-full h-full p-0 m-0 bg-transparent relative top-[0%] left-[0%]'
                ):
                    self.body_outline_classes = (
                        'bg-transparent h-full absolute top-[0%] left-[0%] p-0 m-0'
                    )
                    self.ui_body_outline = ui.image(
                        f'{self.path_static_img}/ggg_outline_mean_all.svg'
                ...
                with ui.row().classes('w-full h-full p-0 m-0 bg-transparent relative'):
                    ...
                    self.ui_pattern_display = ui.interactive_image('').classes(
    ...
    def create_lights(self, scene: ui.scene, intensity=30.0):
        ...
        light_colors = ['#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff']
        ...
        for i in range(len(light_positions)):
            scene.spot_light(
                ...
                intensity=intensity,
                angle=np.pi,
            ).rotate(0.0, 0.0, -z_dirs[i]).move(
                light_positions[i][0], light_positions[i][1], light_positions[i][2]
            )

    def create_camera(self, cam_location, fov, scale=1.0):
    ...
    def def_3d_scene(self):
        ...
        with ui.row().classes('w-full p-0 m-0 justify-between items-center'):
            ...
            ui.label('INFO: it takes a few minutes').classes(
        ...
        with ui.scene(
            ...
            f'w-[{self.w_garment_display}vw] h-[90%] p-0 m-0'
        ) as self.ui_3d_scene:
            # Lights setup
            self.create_lights(self.ui_3d_scene, intensity=60.0)
    ...
    def def_pattern_waiting(self):
        ...
        with ui.dialog(value=False).props(
            ...
            ui.spinner('hearts', size='15em').classes(
                'fixed-center'
            )  # NOTE: 'dots' 'ball'
    ...
    def def_body_file_dialog(self):
        ...
        with ui.dialog() as self.ui_body_dialog, ui.card().classes('items-center'):
            ...
            ui.upload(
                label='Body parameters .yaml or .json', on_upload=handle_upload
    ...
    def def_design_file_dialog(self):
        ...
        async def handle_upload(e: events.UploadEventArguments):
            ...
            self.toggle_param_update_events(
                self.ui_design_refs
            )  # Don't react to value updates
            ...
            self.toggle_param_update_events(
                self.ui_design_refs
            )  # Re-enable reaction to value updates
        ...
        with ui.dialog() as self.ui_design_dialog, ui.card().classes('items-center'):
            # NOTE: https://www.reddit.com/r/nicegui/comments/1393i2f/file_upload_with_restricted_types/
            ui.upload(
                label='Design parameters .yaml or .json', on_upload=handle_upload
            ).classes('max-w-full').props('accept='.yaml,.json'')
    ...
    def def_parse_tab(self):
        ...
        with ui.column().classes('w-full h-full p-0 m-0'):
            ...
            with ui.column().classes('w-full h-[80vh] p-4 gap-4'):
                # Header text
                ui.label('Describe your design or upload reference images').classes(
                    'text-lg font-medium text-gray-700'
                )

                # Messages will be added here
                self.chat_container = ui.column().classes(
                    'w-full flex-grow overflow-auto gap-4'
                )
            ...
            with ui.row().classes('w-full gap-4 items-center bg-white border-t p-4'):
                ...
                ui.button(
                    icon='attach_file', on_click=lambda: self.upload_dialog.open()
                ).props('flat round')

                self.chat_input = (
                    ui.input(placeholder='Describe your design...')
                    .props('outlined')
                    .classes('flex-grow')
                )

                ui.button('Send', on_click=self.handle_chat_input).props(
                    f'color={theme_colors.primary}'
                ).classes('px-6')
        ...
        with self.chat_container:
            with ui.card().classes('w-3/4 ml-auto bg-gray-200 rounded-lg'):
                with ui.column().classes('p-3 gap-1'):
                    ...
                    ui.label(datetime.now().strftime('%H:%M')).classes(
        ...
        try:
            ...
            with self.chat_container:
                with ui.card().classes('w-3/4 mr-auto bg-white rounded-lg'):
                    with ui.column().classes('p-3 gap-1'):
                        ui.label(
                            'I've updated the pattern based on your description. You can adjust the parameters further if needed.'
            ...
            with self.chat_container:
                with ui.card().classes('w-3/4 mr-auto bg-red-100 rounded-lg'):
                    with ui.column().classes('p-3 gap-1'):
                        ui.label(
                            'Sorry, the request took too long. Please try again with a simpler description.'
                        ).classes('text-gray-800')
                        ui.label(datetime.now().strftime('%H:%M')).classes(
                            'text-xs text-gray-500'
                        )
            ...
            with self.chat_container:
                with ui.card().classes('w-3/4 mr-auto bg-red-100 rounded-lg'):
                    with ui.column().classes('p-3 gap-1'):
                        ui.label(
                            'Sorry, I couldn't process that input. Please try again.'
                        ).classes('text-gray-800')
                        ui.label(datetime.now().strftime('%H:%M')).classes(
                            'text-xs text-gray-500'
                        )
    ...
    async def handle_image_upload(self, e: events.UploadEventArguments):
        ...
        try:
            ...
            self.content_type = e.type or 'image/png'
            ...
            params = await loop.run_in_executor(
                self._async_executor,
                lambda: self.pattern_parser.process_input(
            ...
            with self.chat_container:
                with ui.card().classes('w-3/4 mr-auto bg-white rounded-lg'):
                    with ui.column().classes('p-3 gap-1'):
                        ui.label(
                            'I've updated the pattern based on your reference image. You can adjust the parameters further if needed.'
                        ).classes('text-gray-800')
                        ui.label(datetime.now().strftime('%H:%M')).classes(
                            'text-xs text-gray-500'
                        )
    ...
    async def update_pattern_ui_state(
        self, param_dict=None, param=None, new_value=None, body_param=False
    ):
    ...
    def _sync_update_state(self):
        ...
        self.update_body_params_ui_state(
            self.ui_passive_body_refs
        )  # Display evaluated dependencies
        ...
        if self.ui_pattern_display is not None:
            ...
            if self.pattern_state.svg_filename:
                ...
                w_shift = abs(
                    p_bbox[0]
                )  # Body feet location in width direction w.r.t top-left corner of the pattern
                m_top = (
                    1.0 - abs(p_bbox[2]) * self.background_body_scale
                ) * self.h_rel_body_size + (1.0 - self.h_rel_body_size) / 2
                m_left = (
                    self.background_body_canvas_center
                    - w_shift * self.background_body_scale * self.w_rel_body_size
                )
                m_right = (
                    1
                    - m_left
                    - p_bbox_size[0] * self.background_body_scale * self.w_rel_body_size
                )
                m_bottom = (
                    1
                    - m_top
                    - p_bbox_size[1] * self.background_body_scale * self.h_rel_body_size
                )
                ...
                if m_top < 0 or m_bottom < 0 or m_left < 0 or m_right < 0:
                    ...
                    scale_margin = 1.2
                    y_top_scale = abs(min(m_top * scale_margin, 0.0)) + 1.0
                    y_bot_scale = 1.0 + abs(min(m_bottom * scale_margin, 0.0))
                    x_left_scale = abs(min(m_left * scale_margin, 0.0)) + 1.0
                    x_right_scale = abs(min(m_right * scale_margin, 0.0)) + 1.0
                    scale = min(
                        1.0 / y_top_scale,
                        1.0 / y_bot_scale,
                        1.0 / x_left_scale,
                    ...
                    self.ui_body_outline.classes(
                        replace=self.body_outline_classes
                        + f' origin-center scale-[{scale}]'
                    )
                    ...
                    m_top = (
                        1.0 - abs(p_bbox[2]) * self.background_body_scale
                    ) * self.h_rel_body_size * scale + (
                        1.0 - self.h_rel_body_size * scale
                    ) / 2
                    m_left = (
                        (0.5 - body_center * scale)
                        - w_shift
                        * self.background_body_scale
                        * self.w_rel_body_size
                        * scale
                    )
                    m_right = (
                        1
                ...
                self.ui_pattern_display.set_source(
                    str(self.pattern_state.svg_path())
                    if self.pattern_state.svg_filename
                    else ''
                )
    ...
    async def enhace_prompt(self):
        ...
        def prompt_enhancer(): 
            # this will return the same text as of now.. need to be updated
            # TODO work in enhancing the prompt and need to return it
            print('\n\n\nPrompt is now enhanced and sent again\n\n\n')
            return text
        
        enhanced_prompt = prompt_enhancer() # new AI integration can be written inside the above funstion
    ...
    def _sync_update_3d(self):
        ...
        self.loop.create_task(self.enhace_prompt())
    ...
    async def design_sample(self):
        ...
        self.loop = asyncio.get_event_loop()
        await self.loop.run_in_executor(
            self._async_executor, self.pattern_state.sample_design
        )
    ...
    async def random(self):
        ...
        self.toggle_param_update_events(
            self.ui_design_refs
        )  # Re-do reaction to value updates
    ...
    def state_download(self):
        ...
        ui.download(
            archive_path,
            f'Configured_design_{datetime.now().strftime('%y%m%d-%H-%M-%S')}.zip',
        )
